...
%Macro Charges;
	data modele.rev_imp&anr1.;
		...
		/* A4	DEDUCTIONS DIVERSES-------------------------------------------------*/
		DEDd=_6dd;
		/* MODIF ICI : on répartit à 50/50 entre les conjoints */
		if mcdvo in ('M','O') then do;
			DEDd1=DEDd*1/2;
			DEDd2=DEDd*1/2;
			end;
		...
		*/
		...
		/* MODIF ICI : on répartir ces déductions équitablement entre déclarant et conjoint */
		if mcdvo in ('M','O') then do;
			DED_repar1=DED_repar*1/2;
			DED_repar2=DED_repar*1/2;
			end;
		else do;
