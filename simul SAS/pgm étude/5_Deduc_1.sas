...
... */
		...
		/*MODIF 1 : on enlève le plafond pour les enfants et on attribue au déclarant : +&equip_aide_m1_c.*(pchar+0.5*nbh)*/
		plaf_persages=	&equip_aide_m0_c.*(1 +(mcdvo in ('M','O'))) ;
		%if &anleg.<=2015 %then %do ;
...
/********************************************************/
		...
		credit = sum(CRED_conge_agri,
					...
					CRED_PFO,
					CRED_IMPUT); 
		/* MODIF 1 on attribue tous les CI du couple au déclaratn (comme pour rbg_1) */
		credit1 = credit;
		credit2 = 0;
		...
		deduc_ap_decote=sum(RED_garde,
							...
							RED_excep);
		/* MODIF 1 on attribue tous les CI du couple au déclaratn (comme pour rbg_1) */
		deduc_ap_decote1 = deduc_ap_decote;
		deduc_ap_decote2 = 0;
