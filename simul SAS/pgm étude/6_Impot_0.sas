...
%macro Impot;
	...
	%ELSE %IF "&calcul_impot." = "impot_revdisp" %THEN %DO;
		...
		*****************************/
		...
		%AppliqueBaremeIR(rib1,impot11,TxMarg=TxMarginal1);
	...
	%ELSE %IF "&calcul_impot." = "impot_revdisp" %THEN %DO;
		...
		****************************/
		...
		/* MODIF ICI : On crée les impots 4 individuels après les PVP - PVT qu'on répartit entre conjoint et déclarant */
		%do i=1 %to 2;
			%IF "&calcul_impot." = "normal" %THEN %DO;
				impot4&i.=sum(0,impot3&i.,PVP*part&i.,PVT*part&i.);
				%END;
			%ELSE %IF "&calcul_impot." = "impot_revdisp" %THEN %DO;
				impot4&i.=sum(0,impot3&i./*,PVP,PVT*/);
				%END;
			%end;
		%do i=3 %to 5;
			impot4&i.=impot3&i.;
		...
		/* MODIF ICI : On crée les impots 5 individuels après les CI/RI individualisés dans le couple  */
		impot51=max(impot41-deduc_ap_decote1,0);
		impot52=max(impot42-deduc_ap_decote2,0);
		impot53=impot43;
		impot54=impot44;
		impot55=impot45;
		...
		*****************************/
		...
		credit_exceptionnel_09=0; reduc_exceptionnel_17=0;reduc_exceptionnel_171=0;reduc_exceptionnel_172=0;
		%if &anleg.= 2009 %then %do; 
			...
			%end;
		/* MODIF ICI : on individualise les plafonds */
		%if &anleg.=2017 %then %do; /*réduction exceptionnelle 2017 : article 2 de la LOI n° 2016-1917 LFI 2017 (https://www.legifrance.gouv.fr/eli/loi/2016/12/29/ECFX1623958L/jo) */
			seuil1=&excep_lim1_r.;
			seuil2=&excep_lim2_r.;
		/* MODIF ICI :  on répartit le RFR comme le RBG*/
			RFR1 = RFR * part1;
			RFR2 = RFR * part2;
			if RFR1<seuil1 then reduc_exceptionnel_171=&excep_pourcentmax_r.*impot31;
			else if RFR1<seuil2 then reduc_exceptionnel_171=&excep_pourcentmax_r.*impot31*(seuil2-RFR1)/2000; /* Calcul différentiel */
			else reduc_exceptionnel_171=0;
			if RFR2<seuil1 then reduc_exceptionnel_172=&excep_pourcentmax_r.*impot32;
			else if RFR2<seuil2 then reduc_exceptionnel_172=&excep_pourcentmax_r.*impot32*(seuil2-RFR2)/2000; /* Calcul différentiel */
			else reduc_exceptionnel_172=0;
			%end;

		/* MODIF ICI : On crée les impots 7 individuels après crédits (on ne modifie pas pour 2009) */
		impot71=max(0, impot61-reduc_exceptionnel_171)-credit1;
		impot72=max(0, impot62-reduc_exceptionnel_172)-credit2;
		impot73=impot63;
		impot74=impot64;
		impot75=impot65;
		...
		*****************************/
		...
		cred_etr=0;
		/* MODIF ICI : On crée les impots 7 individuels après cred_etre */
		%do i=1 %to 5;
			if _8tk>0 & RnG >0 then cred_etr=(0.9*_8tk/RnG)*impot6&i.;
			impot7&i.=impot7&i.-cred_etr;
			%end;
		...
		/*MODIF ICI : on individualise le plafonnement */
		Avantages_HorsPlafond1= RED_Malraux/2
								+RED_excep/2 
								+RED_dons1*part1
								+RED_dons2*part1
								+RED_mecenat/2
								...
								+RED_syndic/2
								+RED_long_sejour1
								+RED_compta_gestion/2 
								+RED_presta_compens1 
								+RED_rente_survie*part1
								+RED_foret_incendie/2
								+RED_biencult/2
								+CRED_syndic*part1 
								+CRED_conge_agri/2 
								+CRED_persages1
								+CRED_PFO*part1 ;
		AvantageTotal_APlafonner1=credit1+deduc_ap_decote1-Avantages_HorsPlafond1;
		Avantages_HorsPlafond2= RED_Malraux/2
								+RED_excep/2 
								+RED_dons1*part2
								+RED_dons2*part2
								+RED_mecenat/2
								...
								+CRED_persages2
								+CRED_PFO*part2 ;
		AvantageTotal_APlafonner2=credit2+deduc_ap_decote2-Avantages_HorsPlafond2;
		...
		excedant_plafglobal11=	max(AvantageTotal_APlafonner1-RED_Sofica*part1-RED_pinel_dom/2*(&anleg.>=2016)-(&plaf_global_m.+&plaf_global_t.*RNG*part1),0);
		excedant_plafglobal21= 	max(AvantageTotal_APlafonner1-excedant_plafglobal11-(&plaf_global_m.+&plaf_global_m2.+&plaf_global_t.*RNG*part1),0);
		excedant_plafglobal12=	max(AvantageTotal_APlafonner2-RED_Sofica*part2-RED_pinel_dom/2*(&anleg.>=2016)-(&plaf_global_m.+&plaf_global_t.*RNG*part2),0);
		excedant_plafglobal22= 	max(AvantageTotal_APlafonner2-excedant_plafglobal12-(&plaf_global_m.+&plaf_global_m2.+&plaf_global_t.*RNG*part2),0);
		label 	excedant_plafglobal11="Excédant d'avantages fiscaux après application du 1er plafonnement"
				excedant_plafglobal21="Excédant d'avantages fiscaux après application du 2e plafonnement";

		/* Indicatrice */
		plaf_av_fisc1=(excedant_plafglobal11+excedant_plafglobal21>0);
		plaf_av_fisc2=(excedant_plafglobal12+excedant_plafglobal22>0);
		label plaf_av_fisc1="Plafonnement des avantages fiscaux";

		if plaf_av_fisc1 then	impot81=impot71+excedant_plafglobal11+excedant_plafglobal21; 
		else 					impot81=impot71;
		if plaf_av_fisc2 then	impot82=impot72+excedant_plafglobal12+excedant_plafglobal22; 
		else 					impot82=impot72;
		...
		VIII. Contribution exceptionnelle sur les hauts revenus à partir de 2012
		*****************************/
		%Init_Valeur(CEHR CEHR1 CEHR2);
		/*MODIF ICI : on supprime la conjugalisation des plafonds pour les couples et on individualise */
		%if &anleg.>=2012 %then %do;
			if &CEHRplaf1.<RFR*part1
				then CEHR1=(min(RFR*part1,&CEHRplaf2.)-&CEHRplaf1.)*&CEHRtx1.;
			if &CEHRplaf2.<RFR*part1 
				then CEHR1=sum(0,CEHR1,(RFR*part1-&CEHRplaf2.)*&CEHRtx2.);	
			if &CEHRplaf1.<RFR*part2
				then CEHR2=(min(RFR*part2,&CEHRplaf2.)-&CEHRplaf1.)*&CEHRtx1.;
			if &CEHRplaf2.<RFR*part1 
				then CEHR2=sum(0,CEHR2,(RFR*part2-&CEHRplaf2.)*&CEHRtx2.);	
			%end;

		/***************************
		IX. Prélèvements libératoires
		*****************************/
		/*MODIF ICI : on attribue les cases au déclarant et individualise la CEHR*/
		impot91=impot81
				+CEHR1
		...
		X. Mise en recouvrement
		*****************************/
		/* MODIF ICI : impot_i individuels après recouvrement */
		%do i=1 %to 5;
			impot_&i. = impot9&i.;
			if 0<impot9&i.<&recouv. then impot_&i.=0;
			%end;
		...
		impot=impot_1+impot_2+impot_3+impot_4+impot_5;
